...
import { formatEnergyTraded, formatCarbonSavings, formatCurrency } from '@/lib/utils';
...
export function SimulationDashboard() {
  ...
  const fetchSimulationStatus = async () => {
    try {
      const response = await makeAuthenticatedRequest('/api/simulation/status');
      if (response.ok) {
        ...
        toast({
          ...
          description: "Please log in to access the simulation page",
          variant: "destructive"
        });
      }
    } catch (error) {
  ...
  const fetchOptimization = async () => {
    try {
      const response = await makeAuthenticatedRequest('/api/ml/optimization');
      if (response.ok) {
        const data = await response.json();
        ...
        toast({
          title: "Authentication Error", 
          description: "Please log in to access optimization data",
          variant: "destructive"
        });
      }
    } catch (error) {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const response = await makeAuthenticatedRequest('/api/simulation/analytics');
      if (response.ok) {
        const data = await response.json();
        setAnalytics(data);
      } else if (response.status === 401) {
        toast({
          title: "Authentication Error",
          description: "Please log in to access analytics data", 
          variant: "destructive"
        });
      } else {
        // Fallback to network analytics if simulation specific doesn't exist
        const fallbackResponse = await makeAuthenticatedRequest('/api/analytics/network');
  ...
  const startSimulation = async () => {
    setLoading(true);
    try {
      const response = await makeAuthenticatedRequest('/api/simulation/start', { method: 'POST' });
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        toast({
          title: "Authentication Error",
          description: "Please log in to start the simulation",
          variant: "destructive"
        });
      } else {
  ...
  const stopSimulation = async () => {
    setLoading(true);
    try {
      const response = await makeAuthenticatedRequest('/api/simulation/stop', { method: 'POST' });
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        toast({
          title: "Authentication Error",
          description: "Please log in to stop the simulation",
          variant: "destructive"
        });
      } else {
        throw new Error(data.error);
  ...
  const changeWeather = async (condition: string) => {
    setLoading(true);
    try {
      const response = await makeAuthenticatedRequest('/api/simulation/weather', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        toast({
          title: "Authentication Error",
          description: "Please log in to change weather conditions",
          variant: "destructive"
        });
      } else {
        throw new Error(data.error);
  ...
  const triggerOutage = async () => {
    setLoading(true);
    try {
      const response = await makeAuthenticatedRequest('/api/simulation/outage', {
        method: 'POST',
        body: JSON.stringify({})
      });
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        toast({
          title: "Authentication Error",
          description: "Please log in to trigger power outage simulation",
          variant: "destructive"
        });
      } else {
        throw new Error(data.error);
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList className="grid w-full grid-cols-4 gap-1 bg-slate-800/50 border-slate-600/50">
          ...
          <TabsTrigger value="weather" className="text-xs sm:text-sm px-2 py-2">
            <span className="hidden sm:inline">Weather Impact</span>
            <span className="sm:hidden">Weather</span>
          </TabsTrigger>
          <TabsTrigger value="grid" className="text-xs sm:text-sm px-2 py-2">
            <span className="hidden sm:inline">Grid Management</span>
            <span className="sm:hidden">Grid</span>
          </TabsTrigger>
          <TabsTrigger value="equity" className="text-xs sm:text-sm px-2 py-2">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <div className="mt-3 sm:mt-4">
                    <Progress 
                      value={analytics?.equity?.fairDistributionScore || 85} 
                      className="w-full h-2 sm:h-3" 
