...
export class SimulationEngine {
  ...
  public getNetworkStats(): NetworkStats {
    ...
    const averagePrice = recentTrades.length > 0 ? 
      recentTrades.reduce((sum, trade) => sum + (trade.pricePerKwh || 0), 0) / recentTrades.length : 5;
  ...
  private calculateNetworkEfficiency(totalGeneration: number, totalConsumption: number): number {
...
class WeatherSimulator {
  ...
  private getSolarEfficiencyForCondition(condition: WeatherCondition['condition']): number {
    const efficiencies = {
      'sunny': 0.95,
      'partly-cloudy': 0.75,
      'cloudy': 0.45,
